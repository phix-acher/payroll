...
# Usage: Group Payslips according to the years
class WorkingYear(models.Model):
    _name = "hr.working_year"
...
class HrPayslip(models.Model):
    ...
    # Grouping Slips By year
    year = fields.Many2one('hr.working_year', string="Year", copy=False, tracking=True, readonly=True)
    ...
    def _compute_net_salary(self):
        for rec in self:
            if rec.line_ids:
                net = rec.line_ids.filtered(lambda r: r.code == 'NET').mapped('amount')
                rec.net_salary = sum(net)

    def action_print_payslip(self):
        self.ensure_one()
        return self.env.ref('payroll.payslip_details_report').report_action(self)
    ...
    # Grouping Slips By year
    def create_new_year_id(self, year):
        if year:
            new_id = self.env['hr.working_year'].create({
                    'name': year
                })
            return new_id.id
        else:
            raise ValidationError("date value not found!")

    @api.model
    def create(self, vals):
        res = super(HrPayslip, self).create(vals)
        # returns: year form date string (eg. '2025-01-01') : '2025
        year = res.date_to.year

        # get the year record id
        year_id = self.env['hr.working_year'].search([('name', '=', year)]).id or self.create_new_year_id(year)
        if year_id:
            res.year = year_id
